...
def select_bboxes(frame, nbr_object):
    """
    Ask the user to create boxes for the objects on the given frame

    Args:
        frame: The frame to display and click on
        nbr_object: The number of objects (and therefore boxes) to localize

    Return:
        A vector of the boxes
    """
    ...
    # Loop to retrieve the boxes and display them during the process.
    while True:
...
def object_tracking(out_video_path, out_txt_path, nbr_object, start_frame, end_frame=None):
    """
    Perform tracking of the given object specified by the user, 
    using the YOLO detector and Kalman filter
    
    Args:
        out_path: The path of the video showing the tracking
        log_path: The path of the trajectory file.txt of the objects
        nbr_object: The number of object to track
        start_frame: The starting frame on the IN_PATH video (in config)
        end_frame: The ending frame on the IN_PATH video (in config)
    """
    ...
    try:
        ...
        traj_txt = open(out_txt_path, "w")
        traj_txt.write(f"nbr_object : {nbr_object}\n")
        traj_txt.write("Frame,ID,cx,cy,w,h\n")
        ...
        for frame_idx in range(start_frame, end_frame+1):
            ...
            if detector.have_something():
                ...
                # Compute the cIoU score between all pair of tracker and detection
                all_ciou_per_det = np.zeros( (len(detector.boxes_cc), nbr_object), dtype=float)
                for tid, trk in enumerate(trackers):
                    if trk.is_active():
                        trk.predict()

                    for deti, det in enumerate(detector.boxes_xy):
                        ciou_score = ciou(cxcywh_to_xyxy(trk.last_pred), det)
                        all_ciou_per_det[deti][tid] = ciou_score

                # Loop to give the best corresponding detection to the corresponding tracker
                best_matches = []
                for ciou_vec, det in zip(all_ciou_per_det, detector.boxes_cc):
                    best_match = np.argmax(ciou_vec)

                    # Check to not give 2 detection to the same tracker
                    if best_match not in best_matches:
                        trackers[best_match].update(det)
                    best_matches.append(best_match)

            # Draw for video, write the txt and decrement trackers
            ...
            for tid, trk in enumerate(trackers):
                ...
                if trk.is_active():
                    color = (0,255,0)
                    traj_txt.write(f"{frame_idx},{tid},{cx},{cy},{w},{h}\n")
                else:
                    color = (0,0,255)
        ...
        writer.release()
        traj_txt.close()
        print("\nDONE. Tracking video saved:", out_video_path)
...
def backup_tracking(out_video_path, out_txt_path, nbr_object, start_frame, detect_each_x_frame, end_frame=None):
    """
    Backup tracking based solely on manual detection of the object (not automatic and very time-consuming).
    
    Args:
        out_path: The path of the video showing the tracking
        log_path: The path of the trajectory file.txt of the objects
        nbr_object: The number of object to track
        detect_each_x_frame: Ask to annotate each x frame
        start_frame: The starting frame on the IN_PATH video (in config)
        end_frame: The ending frame on the IN_PATH video (in config)
    """
    ...
    try:
        ...
        for frame_idx in range(start_frame, end_frame+1):
            ...
            # Ask to the user next detection
            ...
            # Draw for video and write the txt
            frame_with_pred = frame.copy()
            for tid, trk in enumerate(trackers):
                cx,cy,w,h = trk.kf.statePost[:4].ravel()
                if trk.is_active():
...
def static_tracking(out_video_path, out_txt_path, nbr_object, start_frame, end_frame=None):
    """
    Tracking that can be used for static video, this only asks the user for the positions at the beginning and the end.
    All other positions will be determined through interpolation.
